...
public class MainActivity extends AppCompatActivity {
    ...
    public void onClickOpenAddressButton(View v) {
        Log.d("open map", "ok");
        String address = "1401 John F Kennedy Blvd, Philadelphia, PA"; // city hall
        Uri uri = new Uri.Builder().scheme("geo").path("0,0").appendQueryParameter("q", address)
                .appendQueryParameter("z", "15").build();
        showMap(uri);
    }
    ...
    void showMap(Uri uri) {
        Log.d("showMap", uri.toString());
        Intent intent = new Intent(Intent.ACTION_VIEW);
        intent.setData(uri);
        if (intent.resolveActivity(getPackageManager()) != null) {
            startActivity(intent);
        } else {
            Log.d("showMap", "no map activity available");
        }
    }
}